            ...
            if (jsonMotabarAst(this.responseText))
            {
                ...
                for (let i=0; i<arrObjNatijeh.length; i++)
                {
                    ...
                    lmnDasteh.innerHTML = '<div class="shomJDST"></div>\n' +
                        ...
                        '    <div class="onvanJDST" onclick="if (mobileAst(1000)) namayeshPeygham(\''+arrObjNatijeh[i]["onvan"] + ' (استفاده در ماه: ' + momayezdar(arrObjNatijeh[i]["tedadMah"]) + ' , استفاده در سال: ' + momayezdar(arrObjNatijeh[i]["tedadSal"]) + ' , استفاده در کل: ' + momayezdar(arrObjNatijeh[i]["tedadKol"]) + ')' +'\')">'+ arrObjNatijeh[i]["onvan"] +'</div>\n' +
...
function sabtVirayeshDST(id)
{
    ...
    xhttp.onreadystatechange = function ()
    {
        if (this.readyState === 4 && this.status === 200)
        {
            ...
            else if (this.responseText === "er:tool") namayeshPeygham("ویرایش با خطا مواجه شد! نام دسته نباید بیشتر از 30 کاراکتر باشد.");
...
function sabtDasteh()
{
    ...
    xhttp.onreadystatechange = function ()
    {
        if (this.readyState === 4 && this.status === 200)
        {
            ...
            else if (this.responseText === "er:tool") namayeshPeygham("ثبت با خطا مواجه شد! نام دسته نباید بیشتر از 30 کاراکتر باشد.");
